<script>
  ...
  import { FEATURED_APPS, POPULAR_APPS } from "../lib/sample.js";
  ...
  let shown = $derived(category === "all" ? POPULAR_APPS : POPULAR_APPS.filter((a) => a.subtitle === category));
  let rows = $derived(Math.max(1, Math.ceil(shown.length / 3)));
</script>

<FeatureBand slides={FEATURED_APPS} height={380} onaction={(s) => openTarget(s.url)} />

<div class="content">
  <Chips items={CATEGORIES} value={category} onchange={(v) => (category = v)} label="App categories" />

  <section class="chart">
    <h2 class="sect">Popular</h2>
    {#if shown.length}
