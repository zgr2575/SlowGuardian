...
try {
  gitCommitHash = execSync("git rev-parse HEAD", { cwd: rootDir })
    .toString()
    .trim();
  gitCommitShort = execSync("git rev-parse --short HEAD", { cwd: rootDir })
    .toString()
    .trim();
  gitBranch = execSync("git rev-parse --abbrev-ref HEAD", { cwd: rootDir })
    .toString()
    .trim();
  gitCommitDate = execSync("git log -1 --format=%cI", { cwd: rootDir })
    .toString()
    .trim();
  gitCommitMessage = execSync("git log -1 --format=%s", { cwd: rootDir })
    .toString()
    .trim();
} catch (error) {
...
console.log(
  `🔗 Commit: ${buildInfo.git.commitShort} (${buildInfo.git.commitMessage})`
);
