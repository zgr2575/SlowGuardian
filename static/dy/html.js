...
(() => {
        ...
        : e
            ...
            .map((a) =>
              a
                .split(" ")
                .map((s, c) =>
                  c == 0 ? t.url.encode(s, t.baseURL || t.meta) : s,
                )
                .join(" "),
            )
            .join(", ");
  ...
  function be(e, t, a, s, c = "", T = !1, l = "") {
    ...
    if (T) {
          ...
          attrs: [
            {
              name: "src",
              value:
                e + (m ? "?" + Math.floor(Math.random() * 89999 + 1e4) : ""),
            },
          ],
        },
        {
          ...
          attrs: [
            {
              name: "src",
              value:
                t + (m ? "?" + Math.floor(Math.random() * 89999 + 1e4) : ""),
            },
          ],
        },
      ];
            ...
            attrs: [
              {
                ...
                value:
                  "data:application/javascript;base64," +
                  btoa(c + ";document.currentScript?.remove();"),
              },
            ],
          }),
        l &&
            ...
            attrs: [
              {
                ...
                value:
                  "data:application/javascript;base64," +
                  btoa(l + ";document.currentScript?.remove();"),
              },
            ],
          }),
        _
      ...
      return (
        this.ctx.config.assets.files.inject &&
          f.unshift(
            `<script src="${this.ctx.config.assets.files.inject + (m ? "?" + Math.floor(Math.random() * (99999 - 1e4) + 1e4) : "")}"><\/script>`,
          ),
        s &&
          f.unshift(
  ...
  function Xt(e) {
    return (
      (e >= R.UPPER_A && e <= R.UPPER_F) || (e >= R.LOWER_A && e <= R.LOWER_F)
    );
  }
  function qt(e) {
    return (
  ...
  var Le = class {
    ...
    stateNamedEntity(t, a) {
      ...
      for (; a < t.length; a++, this.excess++) {
        ...
        if (
          ((this.treeIndex = pe(s, c, this.treeIndex + Math.max(1, T), l)),
          this.treeIndex < 0)
        )
        ...
        if (
          ((c = s[this.treeIndex]), (T = (c & P.VALUE_LENGTH) >> 14), T !== 0)
        ) {
          if (l === R.SEMI)
    ...
    emitNamedEntityData(t, a, s) {
      ...
      return (
        this.emitCodePoint(a === 1 ? c[t] & ~P.VALUE_LENGTH : c[t + 1], s),
  ...
  var h = u,
    Xe = {
      ...
      [E.MATHML]: new Set([h.MI, h.MO, h.MN, h.MS, h.MTEXT, h.ANNOTATION_XML]),
  ...
  var X = class {
    ...
    _leaveAttrName() {
      ...
      if (re(s, this.currentAttr.name) === null) {
        if (
          (s.attrs.push(this.currentAttr), s.location && this.currentLocation)
        ) {
          let c =
    ...
    _stateAttributeName(t) {
      switch (t) {
        ...
        default:
          this.currentAttr.name += String.fromCodePoint(Q(t) ? ne(t) : t);
      }
    }
          ...
          : this._consumeSequenceIfMatch(g.CDATA_START, !0)
            ? this.inForeignNode
              ...
              : (this._err(d.cdataInHtmlContent),
                this._createCommentToken(g.CDATA_START.length + 1),
    ...
    _stateAmbiguousAmpersand(t) {
      Oe(t)
        ...
        : (t === r.SEMICOLON && this._err(d.unknownNamedCharacterReference),
          this._reconsumeInState(this.returnState, t));
    }
      ...
      hasInListItemScope(t) {
        for (let a = this.stackTop; a >= 0; a--) {
          let s = this.tagIDs[a],
            c = this.treeAdapter.getNamespaceURI(this.items[a]);
          if (s === t && c === E.HTML) return !0;
          if (((s === u.UL || s === u.OL) && c === E.HTML) || ce.get(s) === c)
            return !1;
        }
        return !0;
      }
      hasInButtonScope(t) {
        for (let a = this.stackTop; a >= 0; a--) {
          let s = this.tagIDs[a],
            c = this.treeAdapter.getNamespaceURI(this.items[a]);
          if (s === t && c === E.HTML) return !0;
          if ((s === u.BUTTON && c === E.HTML) || ce.get(s) === c) return !1;
        }
        return !0;
      }
      hasInTableScope(t) {
        for (let a = this.stackTop; a >= 0; a--) {
          let s = this.tagIDs[a];
          if (this.treeAdapter.getNamespaceURI(this.items[a]) === E.HTML) {
            if (s === t) return !0;
            if (s === u.TABLE || s === u.TEMPLATE || s === u.HTML) return !1;
          }
        }
        return !0;
      }
      hasTableBodyContextInTableScope() {
        for (let t = this.stackTop; t >= 0; t--) {
          ...
          if (this.treeAdapter.getNamespaceURI(this.items[t]) === E.HTML) {
            if (a === u.TBODY || a === u.THEAD || a === u.TFOOT) return !0;
  ...
  var Je = { type: D.Marker },
    de = class {
      ...
      insertElementAfterBookmark(t, a) {
        ...
        this.entries.splice(s, 0, { type: D.Element, element: t, token: a });
      }
      removeEntry(t) {
      ...
      getElementEntryInScopeWithTagName(t) {
        let a = this.entries.find(
          (s) =>
            s.type === D.Marker || this.treeAdapter.getTagName(s.element) === t,
        );
  ...
  var it = { TEXT_HTML: "text/html", APPLICATION_XML: "application/xhtml+xml" },
    Eu = "definitionurl",
    Tu = "definitionURL",
    hu = new Map(
      [
  ...
  function nt(e) {
    ...
    return (
      (t === u.FONT &&
        e.attrs.some(
          ({ name: s }) => s === B.COLOR || s === B.SIZE || s === B.FACE,
        )) ||
      mu.has(t)
    );
  }
  ...
  function Au(e, t, a) {
    ...
    return (
      t === E.SVG && (e === u.FOREIGN_OBJECT || e === u.DESC || e === u.TITLE)
    );
  }
  function ot(e, t, a, s) {
    return (
    ...
    J = class {
      ...
      _initTokenizerForFragmentParsing() {
        if (
          !(
            !this.fragmentContext ||
            this.treeAdapter.getNamespaceURI(this.fragmentContext) !== E.HTML
          )
        )
      ...
      _setDocumentType(t) {
        let a = t.name || "",
          ...
          c = t.systemId || "";
        if (
          (this.treeAdapter.setDocumentType(this.document, a, s, c), t.location)
        ) {
      ...
      _reconstructActiveFormattingElements() {
        ...
        if (t) {
          let a = this.activeFormattingElements.entries.findIndex(
              (c) =>
                c.type === D.Marker || this.openElements.contains(c.element),
            ),
            s = a < 0 ? t - 1 : a - 1;
          for (let c = s; c >= 0; c--) {
  ...
  function wa(e, t) {
    ...
    switch (a) {
      ...
      case u.THEAD:
      case u.TR: {
        e.openElements.hasInTableScope(a) && (e._closeTableCell(), Lt(e, t));
        break;
      }
      case u.BODY:
      case u.CAPTION:
    ...
    if (t.tagID === u.HTML) {
      if (
        ...
        s &&
          !(
            !(
              (a = e.treeAdapter.getNodeSourceCodeLocation(s)) === null ||
              a === void 0
            ) && a.endTag
          ) &&
  ...
  self.html = class {
    constructor(t) {
      ...
      this.config = [
        ...
        { elements: ["a", "link", "area"], tags: ["href"], action: "url" },
...
<A HREF="${t}">here</A>.
    ...
    rewrite(t, a, s = []) {
      ...
      if (
        ...
        (l = l.map(
          (N) => (
            (N.attribs = {}),
            N.attrs &&
              N.attrs.map(({ name: ye, value: kt }) => (N.attribs[ye] = kt)),
            N
          ),
        )),
          ...
          _.nodeName == "script" &&
            ...
            !f.getAttribute("src") &&
              ...
              _.childNodes.forEach((N) => {
                ...
                if (
                  f.getAttribute("type") &&
                  f.getAttribute("type") !== "application/javascript" &&
                  f.getAttribute("type") !== "text/javascript" &&
            ...
            for (var b of L.tags)
              if (!(!f.hasAttribute(b) || !f.getAttribute(b))) {
                ...
                if (L.action === "url") {
                  if (
                    (f.setAttribute(`data-dynamic_${b}`, f.getAttribute(b)),
                    !f.getAttribute(b).match(c.ctx.regex.ProtocolRegex) &&
                      f.getAttribute(b).match(/^([a-zA-Z0-9\-]+)\:\/\//g))
                  )
                    continue;
                  ...
                  f.setAttribute("src", URL.createObjectURL(N));
                } else if (L.action === "http-equiv") {
                  let N = f.getAttribute("content");
                  switch (f.getAttribute("http-equiv").toLowerCase()) {
                    case "refresh":
                      var Ut = N.split("url=")[0].split(";")[0],
                        Ft = N.split("url=")[1];
                      f.setAttribute(
                        "content",
                  ...
                  L.action === "css"
                    ? (f.setAttribute(`data-dynamic_${b}`, f.getAttribute(b)),
                      f.setAttribute(
                        b,
                        c.ctx.rewrite.css.rewrite(f.getAttribute(b), m),
                      ))
                    : L.action === "delete"
                      ? f.removeAttribute(b)
                      : L.action === "js" &&
      ...
      if (s && T.childNodes.length && s.length)
        for (
          var ke = T.childNodes.find((N) => N.nodeName == "html"), w = 0;
          w < s.length;
          w++
        ) {
          if (ke) {
