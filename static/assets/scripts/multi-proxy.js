...
class MultiProxySystem {
  ...
  loadProxyPreferences() {
    ...
    if (
      savedProxy &&
      this.proxies[savedProxy] &&
      this.proxies[savedProxy].enabled
    ) {
  ...
  enableStealthMode() {
    ...
    this.failoverQueue = this.failoverQueue.filter(
      (proxy) => this.proxies[proxy].stealth
    );
  ...
  applyStealthConfigurations() {
    ...
    if (Math.random() > 0.7) {
      const stealthProxies = this.failoverQueue.filter(
        (p) => this.proxies[p].stealth
      );
      if (stealthProxies.length > 1) {
        this.currentProxy =
          stealthProxies[Math.floor(Math.random() * stealthProxies.length)];
      }
    }
  ...
  getNextAvailableProxy() {
    const available = this.failoverQueue.filter(
      (proxy) => this.proxies[proxy].enabled && proxy !== this.currentProxy
    );
