...
class CustomDropdown {
  constructor(element) {
    ...
    this.selectedText = element.querySelector(`option[value="${this.selectedValue}"]`)?.textContent || element.querySelector('option').textContent;
    ...
    document.addEventListener('keydown', (e) => {
      if (this.isOpen) {
        switch (e.key) {
          ...
          case 'Enter':
            e.preventDefault();
            const focused = this.optionsElement.querySelector('.dropdown-option.focused');
            if (focused) {
  ...
  close() {
    ...
    this.optionsElement.querySelectorAll('.dropdown-option').forEach(option => {
  ...
  navigateOptions(direction) {
    ...
    const currentFocused = this.optionsElement.querySelector('.dropdown-option.focused');
    
    let currentIndex = currentFocused ? 
      Array.from(options).indexOf(currentFocused) : -1;
  ...
  mutations.forEach((mutation) => {
    mutation.addedNodes.forEach((node) => {
      if (node.nodeType === 1) { // Element node
        const selects = node.querySelectorAll ? node.querySelectorAll('select.form-select') : [];
        selects.forEach(select => {
          if (!select.dataset.customDropdown) {
            new CustomDropdown(select);
            select.dataset.customDropdown = 'true';
