...
class ChatGPTIntegration {
  ...
  addWelcomeMessage() {
    const welcomeMessage = {
      role: 'assistant',
      content: "Hello! I'm your AI assistant. I can help you with web browsing, answer questions, or assist with any tasks. How can I help you today?",
      timestamp: Date.now()
    };
  ...
  async sendMessage() {
    ...
    if (!this.apiKey) {
      this.addErrorMessage("API key not configured. Please set your OpenAI API key in Developer Mode settings.");
      return;
    }
  ...
  getCurrentPageContext() {
    ...
    if (window.browserTabs && window.browserTabs.activeTabId) {
      const activeTab = window.browserTabs.tabs.get(window.browserTabs.activeTabId);
      if (activeTab) {
  ...
  prepareMessagesForAPI(userMessage, context) {
    ...
    let systemMessage = "You are a helpful AI assistant integrated into SlowGuardian, a web proxy browser. ";
    ...
    systemMessage += "Provide helpful, accurate, and concise responses. If the user asks about the current page, use the context provided.";
    ...
    const recentMessages = this.messages.slice(-10).filter(msg => msg.role !== 'system');
