document.addEventListener("DOMContentLoaded", function (event) {
  ...
  addTabButton.addEventListener("click", () => {
    ...
    closeButton.addEventListener("click", (event) => {
      ...
      const tabToRemove = tabList.querySelector(
        `[data-tab-id='${newTab.dataset.tabId}']`,
      );
      ...
      if (tabToRemove && iframeToRemove) {
        ...
        if (remainingTabs.length > 0) {
          let indexToActivate = remainingTabs.findIndex(
            (tab) => parseInt(tab.dataset.tabId) > removedTabId,
          );
  ...
  window.addEventListener("message", function (event) {
    ...
    const activeIframe = iframes.find((iframe) =>
    ...
    if (activeIframe) {
      ...
      const tabToUpdate = tabList.querySelector(
        `[data-tab-id='${activeIframe.dataset.tabId}']`,
      );
...
function erudaToggle() {
  ...
  if (erudaWindow.eruda?._isInit) {
    ...
    erudaWindow.document?.head?.appendChild(script) ||
