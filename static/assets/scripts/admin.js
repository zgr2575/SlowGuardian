...
class AdminPanel {
  ...
  initializeElements() {
    ...
    this.accountManagementModal = document.getElementById('account-management-modal');
  ...
  attachEventListeners() {
    ...
    if (this.viewUsersBtn) {
      this.viewUsersBtn.addEventListener('click', () => this.showUserManagement());
    }
    
    if (this.manageAccountsBtn) {
      this.manageAccountsBtn.addEventListener('click', () => this.showAccountManagement());
    }
  ...
  attachModalEventListeners() {
    ...
    if (closeUserModal) closeUserModal.addEventListener('click', () => this.hideModal(this.userManagementModal));
    if (closeUserManagementBtn) closeUserManagementBtn.addEventListener('click', () => this.hideModal(this.userManagementModal));
    if (refreshUsersBtn) refreshUsersBtn.addEventListener('click', () => this.loadUsers());
    if (blockSiteBtn) blockSiteBtn.addEventListener('click', () => this.blockSiteForUser());

    // Account Management Modal
    const closeAccountModal = document.getElementById('close-account-modal');
    const closeAccountManagementBtn = document.getElementById('close-account-management-btn');
    const refreshAccountsBtn = document.getElementById('refresh-accounts-btn');
    const createAdminBtn = document.getElementById('create-admin-btn');

    if (closeAccountModal) closeAccountModal.addEventListener('click', () => this.hideModal(this.accountManagementModal));
    if (closeAccountManagementBtn) closeAccountManagementBtn.addEventListener('click', () => this.hideModal(this.accountManagementModal));
    if (refreshAccountsBtn) refreshAccountsBtn.addEventListener('click', () => this.loadAdminAccounts());
    if (createAdminBtn) createAdminBtn.addEventListener('click', () => this.createAdminAccount());
    ...
    if (closePauseModal) closePauseModal.addEventListener('click', () => this.hideModal(this.pauseModal));
    if (confirmPauseBtn) confirmPauseBtn.addEventListener('click', () => this.pauseSite());
    if (cancelPauseBtn) cancelPauseBtn.addEventListener('click', () => this.hideModal(this.pauseModal));
  }
  ...
  async handleLogin() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        this.showNotification('Successfully logged in to admin panel', 'success');
  ...
  async refreshStats() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        if (this.onlineUsersCount) this.onlineUsersCount.textContent = data.stats.onlineUsers;
        if (this.totalAdminsCount) this.totalAdminsCount.textContent = data.stats.totalAdmins;
        if (this.siteStatus) this.siteStatus.textContent = data.stats.isPaused ? 'Paused' : 'Active';
        
        // Update pause/unpause buttons
        if (data.stats.isPaused) {
          if (this.pauseSiteBtn) this.pauseSiteBtn.style.display = 'none';
          if (this.unpauseSiteBtn) this.unpauseSiteBtn.style.display = 'inline-block';
        } else {
          if (this.pauseSiteBtn) this.pauseSiteBtn.style.display = 'inline-block';
  ...
  async loadUsers() {
    ...
    this.userList.innerHTML = '<div class="loading-text">Loading users...</div>';
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        this.userList.innerHTML = '<div class="error-text">Failed to load users</div>';
      }
    } catch (error) {
  ...
  renderUsers(users) {
    ...
    if (users.length === 0) {
      this.userList.innerHTML = '<div class="no-data">No users currently online</div>';
      return;
    }
  ...
  async blockUser(userId) {
    const reason = prompt('Enter reason for blocking (optional):') || 'Blocked by admin';
  ...
  async loadAdminAccounts() {
    ...
    this.adminList.innerHTML = '<div class="loading-text">Loading accounts...</div>';
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        this.adminList.innerHTML = '<div class="error-text">Failed to load accounts</div>';
      }
    } catch (error) {
  ...
  renderAdminAccounts(admins) {
    ...
    const adminCards = admins.map(admin => `
  ...
  async createAdminAccount() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        this.showNotification(data.error || 'Failed to create account', 'error');
      }
    } catch (error) {
